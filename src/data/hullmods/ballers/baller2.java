 ...
 */
public class baller2 extends BaseHullMod {


    private String getString(String key) {
        return Global.getSettings().getString("der", key);
    }
    public static final float BigProblemo = 40f; // Damage taken %
    public static final float moarfluxtsussy = 20f; // more flux overall %
    public static final float shieldophaso = 25f; // shield eff and phase cost. Phase cost count like this = shiledophaso / 2f
    public static final float xivspiritsupport = 25f; // speed and turn %
    public static final float gayso = 15f; // i don't rememeber

    public String getUnapplicableReason(ShipAPI ship) {
        boolean hasai = false;
        for(String hullmod:ship.getVariant().getHullMods()){
            if(Global.getSettings().getHullModSpec(hullmod).hasTag("ballerinc") && !hullmod.equals("dermondballer2")){
                hasai = true;
            }
        }
        if (hasai){
